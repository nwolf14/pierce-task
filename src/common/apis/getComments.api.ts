...
import { PostsDataKeys, fetchGetWithMutation } from './common/fetchFactory';
...
export const useFetchCommentsData = () => {
  ...
  const { isLoading, isError, mutateAsync } = useMutation({
    mutationFn: async (id: number) =>
      await fetchGetWithMutation(`/posts/${id}/replies?number=1&order_by=date`, isCommentsResponseData),
    onSuccess: (data) => {
      queryClient.setQueryData(PostsDataKeys.Comments, (previousComments: { [key: string]: TComment } = {}) => {
        return {
          ...previousComments,
